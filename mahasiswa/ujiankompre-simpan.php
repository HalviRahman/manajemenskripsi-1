...
if ($uploadOk == 0) {
    ...
    $stmt = $conn->prepare("INSERT INTO ujiankompre (tanggal,nama,nim,bidang,judul,judulskripsi,fileproposal,pembimbing,token)
                            VALUES (?,?,?,?,?,?,?,?,?)");
    $stmt->bind_param("sssssssss", $tanggal, $nama, $nim, $bidang, $judul, $judulskripsi, $proposal, $pembimbing, $token);
    $stmt->execute();
